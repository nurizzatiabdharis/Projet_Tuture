		...
		<div id=corps>
			...
			<?php
				include("../outils/base.php");
					$sql = "SELECT * FROM exo WHERE type='exo_gram_org_mot' AND class='trou'";
					$req = $bd->prepare($sql);
					$req->execute();
					$test=$req->fetchall();
					$req->closeCursor();
			  ...
			  foreach ($test as $champ) {
				if ($key == $champ['id']){
				  if ($value == $champ['reponse']){
					echo "Reponse <strong>vrai</strong> pour la question {$i} <br><br>";
          $i++;
				  }
					else
					{
					  echo "Reponse <strong>faux</strong> pour la question {$i} <br><br>";
					}
				  }
				}
			  }
			...
			?>
			<br><input id=exoeva type="button" value="Refaire la question" onclick="window.location.href='./exo_gram_org_mot.php'">
