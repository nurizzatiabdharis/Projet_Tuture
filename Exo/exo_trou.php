	...
	<?php include('../outils/menu.php');?>
		<div id=corps>
			<section>
				...
				<h3>Compléter les phrases avec le mot juste.</h3>
			<?php
				...
				foreach ($rand_array_rep_trou as $key => $value) {
					...
					$sql = "SELECT * FROM exo WHERE class='trou' AND type='exo_gram' AND idQuestion='{$rand_rep_trou}'";
					$req = $bd->prepare($sql);
					$req->execute();
				...
				echo "<form method='post' action='debug_exo_trou.php'>";
					foreach($test as $un )
					{
