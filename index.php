			...
			<h2> Bienvenue sur le site !</h2>
				...
				<div id="traducteur">
					...
					<form method="post" action="">
						...
						<label for="lang1">Choisissez la langue du mot &agrave; traduire</label>
						<select name="lang1" id="lang1">
							<option value='fr'>Fran&ccedil;ais</option>
							<option value="mal">Malais</option>
						</select>
						<label for="lang2">Choisissez la langue dans laquelle vous voulez le traduire</label>
						<select name="lang2" id="lang2">
							<option value='fr'>Fran&ccedil;ais</option>
							<option value="mal">Malais</option>
						</select>
						<label>Entrez le mot à traduire</label>
						<input type="text" name="target_word">
						<button type="submit" name="translate"> Envoyer </button>
					</form>
				</div>
			</div>
			<table id=auth>
				...
				<tr>
					<td><a href='#'><button class="menu" id="uns_co">Connexion Unice</button></a></td>
				</tr>
				<tr>
				...
				<div id=form> 
					...
					<div id="formUns">
					<h1>Connexion avec votre compte UNS</h1>
					<p> Suite &agrave; un renforcement de la s&eacute;curit&eacute; avec le CAS de Unice, vous ne pourrez pas accéder au site par le biai de votre compte UNS.<br>
						Merci de vous d'utiliser les bouton connexion/inscription pour accéder &agrave; la totalité du site.<br>
						Veuillez nous excuser pour le désagrément.</p>
						<form method='post' action='outils/cas_authentification.php'>
							<div class="row">
								<div class="col-25">
									<label for=mailUns>Email</label>
								</div>	
								<div class="col-75">
									<input type=email name=mailUns id=mailUns required>
								</div>	
							</div>
							<div class="row">
								<div class="col-25">
									<label for=mdpUns> Mot de passe </label>
								</div>
								<div class="col-75">
									<input type=password name=mdpUns id=mdpUns required>
								</div>
							</div>
							<br><br>
							<button type=submit id="btninfo">Connexion</button>
							<button type=reset id="btninfo">Reset</button>
						</form>
					</div>
				</div>
				<div class='bottom-container maxWidth'>
					 ...
					 if(isset($_GET['error']))
					{
						...
						{
							$cpt++;
							echo "<div id='warning{$cpt}' class='inside_bottom-container maxWidth'>
								<p style='margin:5px 0 15px 15px; float: left'>Vous devez vous connecter pour pouvoir accèder à cette page</p>
								<p style='margin:5px 15px 15px 0; float: right'><button id='close{$cpt}' onclick='closeDiv(this.id)'>Fermer</button></p>
							</div>";
						}
						...
						if($_GET['error']=='forbidden'){
							$cpt++;
							echo "<div id='warning{$cpt}' class='inside_bottom-container maxWidth'>
								<p style='margin:5px 0 15px 15px; float: left'>Vous avez tent&eacute; d'acc&egrave;der &agrave; une page alors que vous n'y &ecirc;tes pas autoris&eacute;</p>
								<p style='margin:5px 15px 15px 0; float: right'><button id='close{$cpt}' onclick='closeDiv(this.id)'>Fermer</button></p>
							</div>";
						}
					} ?>
